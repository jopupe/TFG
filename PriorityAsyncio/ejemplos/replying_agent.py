...
async def main():
    ...
    for j in range (0,num_experiments): 
        ...
        receiver_agent = ReceiverAgent("agent_100@localhost", "test", priority = -5, ag_name="Agent_R")
        await receiver_agent.start()

        sender_agents = [SenderAgent(f"agent_{i}@localhost", "test", priority = -1, ag_name= f"Agent_S{i}") for i in range(1, 21)]
